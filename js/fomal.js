...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Infinity🍫")[0];
}
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://s21.ax1x.com/2025/10/05/pV7MgR1.png",
        hoverMsg: "不能拖呦",
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Infinity WXY %c 你正在访问个人博客，欢迎交流！",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Infinity🍫") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Infinity🍫的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("09/05/1997 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(3979640400 + ((now - start) / 1000) * 23.9995); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("10/04/2025 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://s21.ax1x.com/2024/11/02/pADOblj.png' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://s21.ax1x.com/2024/11/02/pADOHpQ.png' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">NO.6 击败全国 0.01% 的用户🤢</span>`
      } else if (fps <= 15) {
        var kd = `<span style="color:red">NO.5 帧率较低😖</span>`
      } else if (fps <= 25) {
        var kd = `<span style="color:orange">NO.4 中度卡顿😨</span>`
      } else if (fps < 35) {
        var kd = `<span style="color:#9338e6">NO.3 轻微卡顿🙄</span>`
      } else if (fps <= 45) {
        var kd = `<span style="color:#08b7e4">NO.2 帧率较高😁</span>`
      } else {
        var kd = `<span style="color:#39c5bb">NO.1 十分流畅🤣</span>`
      }
...
function setFont(n) {
  ...
  if (n == "default") {
    document.documentElement.style.setProperty('--global-font', '-apple-system');/*10/4change*/
...
if (localStorage.getItem("transNum") == undefined) {
  localStorage.setItem("transNum", 70);
}
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    ...
    --mobileday-bg: url(https://bing.img.run/1920x1080.php);
    --mobilenight-bg: url(https://bing.img.run/1920x1080.php);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 星空特效 </div><input type="checkbox" id="universeSet" onclick="setUniverse()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> 霓虹彩灯 </div><input type="checkbox" id="lightSet" onclick="setLight()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 模糊效果 </div><input type="checkbox" id="blur" onclick="setBlur()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> 侧边栏 </div><input type="checkbox" id="rightSideSet" onclick="toggleRightside()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 帧率监测 </div><input type="checkbox" id="fpson" onclick="fpssw()">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 雪花特效 </div><input type="checkbox" id="snowSet" onclick="setSnow()">
</div>


<h2>字体设置</h2>
<div class="note warning modern"><p>非商免字体未经授权只能个人使用。如有侵权请联系。 —— 致版权方</p>
</div>
<p id="swfs">
...
<details class="folding-tag" cyan><summary> 查看壁纸API系列背景 </summary>
              <div class='content'>
              <div class="bgbox"><a id="bingDayBox" rel="noopener external nofollow" style="background-image: ${bingDayBg}" class="box apiBox" onclick="changeBg('${bingDayBg}')"></a><a id="bingHistoryBox" rel="noopener external nofollow" style="background-image: ${bingHistoryBg}" class="box apiBox" onclick="changeBg('${bingHistoryBg}')"></a></div>
              </div>
            </details>
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://bing.img.run/1920x1080.php"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
